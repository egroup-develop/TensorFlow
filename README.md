...
- 第13回
  - 前回との大きな変更点
    - 膨大な数のデータセットを投入すると, loss関数での計算が無限に発散(∵ 0/0)するエラーが出ていたので, クロスエントロピーの計算部に0を投入しないようソフトマックス関数の出力が(1e^-10, 1]に収まるようにした
    - データセットの数と質
  - データセット: デーロル12/12までの328人の画像, 1人4枚
    - 正規化: 1枚に対してコントラスト強調してから顔の直立補正(45度 ~ 135度まで9度ずつ回転)
    - 訓練: 3枚 x 11枚 x 328人 = 10824枚
    - 評価: 1枚 x 11枚 x 328人 = 3608枚
  - ニューラルネット
    - 構造: C->P->ReLU x2回->Fc->ReLU->Drop->Fc->S
    - 学習率: 0.01
    - 入力: 28x28x3
    - ステップ数: 200
    - バッチ数: 100
  - 学習
    - 所用時間: 8時間くらい
      - step 196, training accuracy 0.997967
step 197, training accuracy 0.997967
