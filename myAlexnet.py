...
###############################################
def inference(images_placeholder, weights, biases, keep_prob):
    # 入力画像(Tensor)を28*28*3にリシェイプ
    images_placeholder = tf.reshape(images_placeholder, shape=[-1, 28, 28, 3])

    ##########畳み込み層##########
    # 畳み込み
    with tf.name_scope('conv1') as scope:
        conv1 = conv2d('conv1', images_placeholder, weights['wc1'], biases['bc1'])
    ...
    ###########出力層##########
    output = tf.nn.softmax(tf.matmul(dense2, weights['out']) + biases['out'])
    return tf.clip_by_value(output, 1e-10, 1.0)
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        logits = inference(images_placeholder, weights, biases, keep_prob)
        # loss()を呼び出して損失を計算
